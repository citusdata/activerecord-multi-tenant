    ...
    module ClassMethods
      def acts_as_distributed(tenant, options = {})
        ...
        if to_s.underscore.to_sym != tenant
          ...
          # TODO: Use with_id here once we have it
          around_save -> (record, block) { persisted? ? ActsAsDistributed.with(record.public_send(tenant)) { block.call } : block.call }
          around_update -> (record, block) { ActsAsDistributed.with(record.public_send(tenant)) { block.call } }
          around_destroy -> (record, block) { ActsAsDistributed.with(record.public_send(tenant)) { block.call } }
        end
