    ...
    def initialize(connection, options = {})
      ...
      @distribution_columns =
        if citus_version.present?
          @connection.execute('SELECT logicalrelid::regclass AS table_name, column_to_column_name(logicalrelid, '+
                                'partkey) AS dist_col_name FROM pg_dist_partition').to_h do |v|
