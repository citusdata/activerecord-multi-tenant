...
describe MultiTenant, 'Sidekiq' do
  ...
  describe 'server middleware' do
    it 'sets the multi-tenant context when provided in message' do
    ...
    it 'sets the multi-tenant context (id) even if tenant not found' do
      server.call(double, { 'bogus' => 'message',
    ...
    it 'does not set the multi-tenant context when no tenant provided' do
