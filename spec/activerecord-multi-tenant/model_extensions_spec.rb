...
describe MultiTenant do
  ...
  describe 'joins for models' do
    context 'for models with where condition in associations' do
      ...
      it 'should add tenant condition to the queries when tenant is set' do
        expected_join_sql = <<-SQL.strip
          SELECT "comments".* FROM "comments" INNER JOIN "tasks"
          ON "tasks"."id" = "comments"."commentable_id" AND "comments"."commentable_type" = 'Task'
          AND "tasks"."account_id" = 1 WHERE "comments"."account_id" = 1
        SQL
      ...
      it 'should add tenant condition to the queries when tenant is not set' do
        MultiTenant.without do
          expected_join_sql = <<-SQL.strip
            SELECT "comments".* FROM "comments" INNER JOIN "tasks"
            ON "tasks"."id" = "comments"."commentable_id" AND "comments"."commentable_type" = 'Task'
            AND "comments"."account_id" = "tasks"."account_id"
          SQL
          expect(Comment.joins(:task).to_sql).to eq(expected_join_sql)
        end
      end
    ...
    context 'for models with default associations' do
      ...
      it 'should add tenant condition to the queries when tenant is set' do
        expected_join_sql = <<-SQL.strip
          SELECT "projects".* FROM "projects" INNER JOIN "tasks"
          ON "tasks"."project_id" = "projects"."id" AND "tasks"."account_id" = 1
      ...
      it 'should add tenant condition to the queries when tenant is not set' do
        MultiTenant.without do
          expected_join_sql = <<-SQL.strip
            SELECT "projects".* FROM "projects" INNER JOIN "tasks"
            ON "tasks"."project_id" = "projects"."id" AND "projects"."account_id" = "tasks"."account_id"
          SQL
          expect(Project.joins(:tasks).to_sql).to eq(expected_join_sql)
        end
      end
  ...
  it 'applies the team_id conditions in the where clause' do
    option1 = <<-SQL.strip
      SELECT "sub_tasks".* FROM "sub_tasks" INNER JOIN "tasks"
      ON "sub_tasks"."task_id" = "tasks"."id" AND "tasks"."account_id" = "sub_tasks"."account_id"
  ...
  it 'tests joins between distributed and reference table' do
    option1 = <<-SQL.strip
      SELECT "categories".* FROM "categories" INNER JOIN "project_categories"
    ...
    MultiTenant.without do
      ...
      expected_sql = <<-SQL.strip
        SELECT "projects".* FROM "projects" INNER JOIN "project_categories"
  ...
  it 'test eager_load' do
    ...
    option1 = <<-SQL.strip
      ...
      "categories"."id" AS t1_r0, "categories"."name" AS t1_r1 FROM "projects"
      LEFT OUTER JOIN "project_categories" ON "project_categories"."project_id" = "projects"."id"
       AND "project_categories"."account_id" = 1 AND "projects"."account_id" = 1
    ...
    option2 = <<-SQL.strip
      SELECT "projects"."id" AS t0_r0, "projects"."account_id" AS t0_r1, "projects"."name" AS t0_r2,
      "categories"."id" AS t1_r0, "categories"."name" AS t1_r1 FROM "projects"
      LEFT OUTER JOIN "project_categories" ON "project_categories"."account_id" = 1
    ...
    MultiTenant.with(account1) do
      project1 = Project.create! name: 'Project 1'
      project_category = ProjectCategory.create! name: 'project cat 1', project: project1, category: category1

      expect(Project.eager_load(:categories).to_sql).to eq(option1).or(eq(option2))

      project = Project.eager_load(:categories).first
      expect(project.categories).to include(category1)
      expect(project.project_categories).to include(project_category)
    end
  ...
  it 'test raw SQL joins' do
    ...
    MultiTenant.with(account1) do
      option1 = <<-SQL.strip
        ...
        LEFT JOIN project_categories pc ON project.category_id = pc.id WHERE "tasks"."account_id" = 1
      SQL
      option2 = <<-SQL.strip
        SELECT "tasks".* FROM "tasks" INNER JOIN "projects" ON "projects"."account_id" = 1
        AND "projects"."id" = "tasks"."project_id" LEFT JOIN project_categories pc ON project.category_id = pc.id
      ...
      expect(Task.joins(:project).joins('LEFT JOIN project_categories pc ON project.category_id = pc.id').to_sql)
        .to eq(option1).or(eq(option2))
    end
    ...
    MultiTenant.without do
      ...
      expect(Task.where(account_id: 1).joins(:project)
                 .joins('LEFT JOIN project_categories pc ON project.category_id = pc.id').to_sql)
        .to eq(expected_sql.strip)
    end
  end
  ...
  it 'only applies clauses when a tenant is set' do
    ...
    MultiTenant.with(account) do
      ...
      # expect(Project).to receive(:find_by_sql)
      # .with(eq(option1).or(eq(option2)).or(eq(option3)), any_args).and_call_original
